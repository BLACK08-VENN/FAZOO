...
-- Retail assignment fixture (array weekly off-days) for the BA flows below.
insert into public.brand_ambassador_assignments
  (id, organization_id, brand_ambassador_id, campaign_id, store_id, weekly_off_day, start_date, status)
values
  ('99999999-9999-4999-8999-999999999901', '11111111-1111-4111-8111-111111111111', '22222222-2222-4222-8222-000000000010', '33333333-3333-4333-8333-333333333301', '44444444-4444-4444-8444-444444444401', ARRAY[0,6]::smallint[], '2000-01-01', 'active');
...
values
  ('cccccccc-cccc-4ccc-8ccc-ccccccccccc2', '11111111-1111-4111-8111-111111111122', '22222222-2222-4222-8222-000000000010', 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaa2', (ARRAY[(extract(dow from (now() at time zone 'Africa/Nairobi'))::int + 1) % 7]::smallint[]), (now() at time zone 'Africa/Nairobi')::date, null, 'active');
...
do $$ begin
  begin
    perform public.ba_checkout(6.6020, 3.3515, '88888888-8888-4888-8888-888888888812', '66666666-6666-4666-8666-666666666612', 'invalid-a.jpg', 'invalid-b.jpg');
...
-- veda_today must expose today's assigned school(s) + stationery catalogue.
do $$ begin
  if (public.veda_today())#>>'{assignments,0,assignment,school_name}' IS DISTINCT FROM 'Safari Academy Test' then
    raise exception 'Veda failure: veda_today did not return the assigned school';
  end if;
  if (public.veda_today())#>>'{assignments,0,is_weekly_off_today}' <> 'false' then
...
do $$ begin
  ...
  begin
    perform public.veda_admin_upsert_assignment(
      ...
      'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaa2', ARRAY[1]::smallint[], current_date, null, 'active', null);
