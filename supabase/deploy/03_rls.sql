...
-- Safety: drop stale policies from prior partial runs
DO $$ DECLARE r RECORD;
BEGIN
  FOR r IN SELECT policyname, tablename FROM pg_policies
           WHERE schemaname = 'public' AND tablename IN (
             'organizations','profiles','campaigns','stores',
             'brand_ambassador_assignments','skus','daily_logs',
             'sales_entries','daily_log_photos','supervisor_scopes',
             'operation_receipts','audit_logs'
           ) LOOP
    EXECUTE format('DROP POLICY IF EXISTS %I ON public.%I', r.policyname, r.tablename);
  END LOOP;
  FOR r IN SELECT policyname FROM pg_policies
           WHERE schemaname = 'storage' LOOP
...
-- ═══════════════════════════════════════════════════════════════════════════
-- Storage — private buckets, path convention {org}/{user}/{file}
-- ═══════════════════════════════════════════════════════════════════════════
...
-- Upload into your own folder within your organization.
-- Folder layout: {organization_id}/{auth_user_id}/{filename}
  ...
  using (
    ...
    and exists (
      ...
      where p.id = auth.uid()
        ...
        and p.role in ('super_admin','organization_admin','supervisor')
    )
  );
