...
begin
  ...
  if p_visit_id is null then
    raise exception 'A school visit is required';
  end if;
  if nullif(btrim(coalesce(p_grade_label, '')), '') is null then
    raise exception 'Enter the grade or class name';
  end if;
  if char_length(btrim(p_grade_label)) > 100 then
    raise exception 'Grade or class name is too long';
  end if;
  if p_copies_requested is null or p_copies_requested < 1 then
    raise exception 'Enter how many copies this grade needs';
  end if;
  ...
  if nullif(btrim(coalesce(p_source_format, '')), '') is null then
    raise exception 'Choose how this grade supplied the booklist';
  end if;
  ...
  if prior is not null and prior->>'grade_request_id' is not null then
    return prior;
  end if;
  ...
  if j.stage in ('declined', 'cancelled', 'completed') then
    raise exception 'This booklist job cannot accept another grade at its current stage';
  end if;

  select sv.id, sv.school_id
    into v
  ...
  select
    coalesce(sum(gr.copies_requested), 0)::integer,
    coalesce(sum(gr.copies_to_print), 0)::integer,
    min(gr.due_date),
    string_agg(gr.grade_label, ', ' order by gr.sort_order, gr.created_at)
  into v_total_requested, v_total_to_print, v_overall_due, v_grade_notes
  from public.booklist_grade_requests gr
  where gr.job_id = p_job_id;

  update public.booklist_jobs
     set copies_requested = v_total_requested,
         copies_to_print = v_total_to_print,
  ...
  v_result := jsonb_build_object(
    ...
    'copies_requested', p_copies_requested,
    'copies_to_print', p_copies_requested + 1,
    'total_copies_requested', v_total_requested,
    'total_copies_to_print', v_total_to_print,
    'due_date', p_due_date,
...
revoke all on function public.ba_submit_grade_booklist(uuid, uuid, text, integer, date, text, uuid, text, bigint, text, integer) from public;
revoke all on function public.ba_submit_grade_booklist(uuid, uuid, text, integer, date, text, uuid, text, bigint, text, integer) from anon;
grant execute on function public.ba_submit_grade_booklist(uuid, uuid, text, integer, date, text, uuid, text, bigint, text, integer) to authenticated;
grant execute on function public.ba_submit_grade_booklist(uuid, uuid, text, integer, date, text, uuid, text, bigint, text, integer) to service_role;
