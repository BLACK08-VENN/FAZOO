-- ═══════════════════════════════════════════════════════════════════════════
-- Fazoo — School booklist pipeline: spreadsheet-board columns.
--
-- Adds the document ids (`raw/formatted/stamped`) and the latest print
-- order's dispatch means to `admin_pipeline_board()` so the admin portal can
-- render a spreadsheet-style view with download links for each school's
-- documents. The function is re-created (dropped and rebuilt via
-- `create or replace`) so applying this migration on top of the already-run
-- `20260910120000` leaves the rest of that migration untouched.
-- ═══════════════════════════════════════════════════════════════════════════
