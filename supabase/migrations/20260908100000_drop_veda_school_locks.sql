-- ═══════════════════════════════════════════════════════════════════════════
-- Fazoo — Remove VEDA school locking.
--
-- All VEDA schools are now accessible to all BAs in the org without
-- passcodes. The veda_school_unlocks table, access_code column on
-- veda_schools, and ba_unlock_veda_school RPC are dropped.
-- ═══════════════════════════════════════════════════════════════════════════

begin;

-- 1. Drop the unlock table (cascades RLS policies)
drop table if exists public.veda_school_unlocks cascade;

-- 2. Drop access_code column from veda_schools
alter table public.veda_schools drop column if exists access_code;

-- 3. Drop the unlock RPC
drop function if exists public.ba_unlock_veda_school(uuid, text);

-- 4. Replace ba_list_veda_schools — remove locked/unlocked fields
...
-- 5. Replace veda_checkin — remove unlock check in self-serve path
create or replace function public.veda_checkin(
...
begin
  ...
  perform public.write_audit(
    ...
    jsonb_build_object(
      ...
      'source', case when p_assignment_id is null then 'self_serve_school' else 'assignment' end
    )
  );
