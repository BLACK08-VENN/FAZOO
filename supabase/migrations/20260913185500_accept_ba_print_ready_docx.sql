...
begin
  ...
  if lower(coalesce(p_storage_path, '')) not like '%.docx' then raise exception 'Upload a print-ready Word .docx file'; end if;
  ...
  if v.id is null or v.school_id is distinct from j.school_id then raise exception 'This visit does not belong to the selected school'; end if;
  if exists (
  ...
  ) then raise exception 'A booklist for this grade or class has already been added'; end if;
  insert into public.booklist_grade_requests (
    ...
    conversion_status, conversion_provider, conversion_finished_at,
    word_storage_bucket, word_storage_path, word_mime_type, word_size_bytes,
    word_published_at, word_published_by
  ) values (
    ...
    coalesce(nullif(p_mime_type, ''), 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'),
    p_file_size_bytes, coalesce(p_sort_order, 0), p.id, p_client_request_id,
    'succeeded', 'ba-upload', now(),
    'booklist-documents', p_storage_path,
    'application/vnd.openxmlformats-officedocument.wordprocessingml.document', p_file_size_bytes,
    now(), p.id
  ) returning id into v_grade_id;
  select min(gr.due_date), string_agg(gr.grade_label, ', ' order by gr.sort_order, gr.created_at)
  ...
  perform public.set_booklist_stage(p_job_id, 'formatted', p.id, 'Print-ready Word booklist uploaded by BA: ' || btrim(p_grade_label));
  perform public.write_audit(
    'booklist_job.grade_request', 'booklist_jobs', p_job_id,
    jsonb_build_object(
  ...
  v_result := jsonb_build_object(
    'status','ok','operation','ba_submit_grade_booklist','job_id',p_job_id,'visit_id',p_visit_id,
    'school_name',j.school_name,'grade_request_id',v_grade_id,'grade_label',btrim(p_grade_label),
    'copies_requested',p_copies_requested,'copies_to_print',p_copies_requested + 1,'due_date',p_due_date,
    'stage','formatted','word_ready',true,'conversion_required',false
  );
