...
export function Input({ className, id, ...props }: InputHTMLAttributes<HTMLInputElement>) {
  return <input id={id} className={cn(baseField, className)} aria-required={props.required} {...props} />;
}

export function Select({ className, id, ...props }: SelectHTMLAttributes<HTMLSelectElement>) {
  return <select id={id} className={cn(baseField, 'pr-8', className)} aria-required={props.required} {...props} />;
}
